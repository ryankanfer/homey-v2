        ...
        <AnimatePresence mode="wait">
          {step === 'accuracy' && (
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              className="w-full max-w-[540px] glass p-6 md:p-10 rounded-sm border-t-4 border-t-[#C8B89A]"
            >
              <div className="text-center mb-8">
                <div className="text-[9px] text-[#A8956E] font-bold tracking-[0.25em] uppercase mb-1.5">
                  Built from your answers
                </div>
                <h2 className="font-serif text-2xl text-[#F0EDE8]">
                  {profile.fullName?.trim() ? `${profile.fullName.split(' ')[0]}'s Profile` : 'Your Profile'}
                </h2>
              </div>

              <div className="space-y-6 mb-12">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5 pb-5 border-b border-[#2A2A27]">
                  <div>
                    <span className="text-[8px] text-[#6E6A65] uppercase tracking-widest block mb-1">
                  ...
                  {(profile.mode === 'Rent' && (profile.meetsIncomeRequirement !== undefined || profile.annualIncome)) && (
                    <div className="col-span-full pt-1">
                      <span className="text-[8px] text-[#6E6A65] uppercase tracking-widest block mb-1">Financial Position</span>
                      <div className="flex items-center gap-3">
                        {profile.annualIncome ? (
                          <span className="font-serif text-lg text-[#C8B89A]">${profile.annualIncome.toLocaleString()}/yr</span>
                        ) : null}
                        {profile.meetsIncomeRequirement ? (
                          <span className="text-[7px] text-[#4A7C59] font-bold uppercase tracking-widest bg-[#4A7C59]/10 px-1.5 py-0.5 border border-[#4A7C59]/20">
                            40x Verified
                          </span>
                        ) : profile.usingGuarantor ? (
                          <span className="text-[7px] text-[#A8956E] font-bold uppercase tracking-widest bg-[#A8956E]/10 px-1.5 py-0.5 border border-[#A8956E]/20">
                            Using Guarantor
                          </span>
                        ) : (
                          <span className="text-[7px] text-[#8B3A3A] font-bold uppercase tracking-widest bg-[#8B3A3A]/10 px-1.5 py-0.5 border border-[#8B3A3A]/20">
                            Below 40x
                          </span>
                        )}
                      </div>
                ...
                <div>
                  <span className="text-[8px] text-[#6E6A65] uppercase tracking-widest block mb-2.5">Territory</span>
                  <div className="flex flex-wrap gap-2">
                    {(profile.territory && profile.territory.length > 0) ? profile.territory.map(t => (
                      <span key={t} className="text-[7px] bg-[#C8B89A]/10 text-[#C8B89A] border border-[#C8B89A]/30 px-2 py-1 uppercase tracking-widest font-bold">
                        {t.split('(')[0].trim()}
                      </span>
                    )) : (
                      <span className="font-serif text-lg text-[#C8B89A]">NYC</span>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-[#0D0D0B] p-5 rounded-sm border border-[#2A2A27]">
                <h3 className="text-center font-serif text-lg text-[#F0EDE8] mb-5">Does this feel accurate?</h3>
                <div className="flex flex-col sm:flex-row gap-2.5">
                  <button
                    onClick={() => handleAccuracy('Yes, this is me')}
                    className="flex-1 py-3 bg-[#C8B89A] text-[#0D0D0B] font-bold text-[9px] uppercase tracking-widest hover:bg-[#E8DCC8] transition-all"
                  >
                    Yes, this is me
                  </button>
                  <button
                    onClick={() => handleAccuracy('Mostly')}
                    className="flex-1 py-3 bg-[#C8B89A]/10 border border-[#C8B89A]/20 text-[#C8B89A] font-bold text-[9px] uppercase tracking-widest hover:bg-[#C8B89A]/20 transition-all"
                  >
                    Mostly
                  </button>
                  <button
                    onClick={() => handleAccuracy('Not quite')}
                    className="flex-1 py-3 bg-[#0D0D0B] border border-[#2A2A27] text-[#6E6A65] font-bold text-[9px] uppercase tracking-widest hover:text-[#F0EDE8] transition-all"
                  >
                    Not quite
                  </button>
                </div>
